        ...
        <div class="col-12">
            <div class="card w-100">
                <div class="card-body p-4">
                    <a  href="{{route('admin.productType')}}"><h5 class="card-title fw-semibold mb-4">Bảng Thương Hiệu</h5></a> 
                    ...
                    <button class="btn btn-secondary light "
                        title="Nhấn để tìm kiếm" type="button" onclick="onAdd()" id="saveEdit">Thêm Thương Hiệu</button>
            ...
            columnDefs: [{
                ...
                },
                {
                    ...
                    render: function(col, type, row) {
                        return (`
                                    <div class="fw-semibold mb-0">${row.brand_name}</div>
                                `);
                    },
                    targets: 1
                },
...
<script>
    ...
    function btnSearch() {
        ...
        history.pushState({}, null, "/admin/brand?search=" + btoa(encodeURIComponent(JSON.stringify(object))));
...
<!-- MODAL EDIT-->
<script>
    const editGetBaseUrl = @json(route('admin.brand.editGet', ['id' => 'ID_PLACEHOLDER']));
    const editPostBaseUrl = @json(route('admin.brand.editPost', ['id' => 'ID_PLACEHOLDER']));
...
<script>
    ...
    function onAdd() {
        currentEditId = null;
    $('#inputTitle').text('Thêm mới thương hiệu');
    $('#formModal')[0].reset();
    $('#dynamicInputs').empty();

    const data = [
        { label: 'Tên thương hiệu', name: 'brand_name', type: 'text', value: '', placeholder: 'Nhập tên', setting: '' },
...
<!-- Delete -->
<script>
    const deleteBaseUrl = @json(route('admin.brand.delete', ['id' => 'ID_PLACEHOLDER']));
