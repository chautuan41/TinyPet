        ...
        <div class="col-12">
            <div class="card w-100">
                <div class="card-body p-4">
                    <a  href="{{route('admin.productType')}}"><h5 class="card-title fw-semibold mb-4">Bảng Loại Sản Phẩm</h5></a> 
                                ...
                                <div class="col-md-6">
                                    <div class="input-group">
                                        <select id="searchSelect" name="searchSelect" class="form-control form-control-sm form-select select2" data-placeholder="Điều kiện tìm kiếm">
                                            ...
                                            <option value="product_type_name">Tên Loại Sản Phẩm</option>
                    ...
                    <button class="btn btn-secondary light "
                        title="Nhấn để tìm kiếm" type="button" onclick="onAdd()" id="saveEdit">Thêm Loại Sản Phẩm</button>
                        ...
                        <table class="table text-nowrap mb-0 align-middle" id="dataTable">
                            <thead class="text-dark fs-4">
                                <tr>
                                    ...
                                    <th class="border-bottom-0">
                                        <h6 class="fw-semibold mb-0">Tên loại sản phẩm</h6>
                                    </th>
                                    <th class="border-bottom-0">
...
<script>
    ...
    function render(data = '') {
        ...
        let datatable = $('#dataTable').DataTable({
            ...
            autoWidth: false,
            ajax: {
                "url": `{{route('admin.productType.data')}}`,
                "dataType": "json",
                "type": "GET",
                "data": data,
                beforeSend: function() {},
            ...
            columnDefs: [{
                    ...
                    render: function(col, type, row, index) {
                        ...
                        return `<td class="border-bottom-0">
                                            <h6 class="fw-semibold mb-0">${row.idPT}</h6>
                                        </td>`;
                ...
                },
                {
                    ...
                    render: function(col, type, row) {
                        return (`
                                    <div class="fw-semibold mb-0">${row.product_type_name}</div>
                                `);
                    },
                    targets: 1
                },
                {
                    ...
                    render: function(col, type, row) {
                        return (`
                            <div class="fw-semibold mb-0">${row.category_name}</div>
                        `);
                    },
                    targets: 2
                },
                ...
                },
                {
                    ...
                    render: function(col, type, row) {
                        ...
                        iconDelete = `<a title="Xóa" class="ms-2" onClick="onDelete('${row.idPT}')" style="cursor: pointer;"><i class="fa-solid fa-trash-can"></i></a>`;
                        ...
                        isEdit = `<a title="Sửa" class="" style="cursor: pointer;" onClick="onEdit(${row.idPT})"><i class="fa-solid fa-pen-to-square"></i></a>`;
...
<script>
    ...
    $('#searchSelect').on('change', function() {
        ...
        category = @json($dataSelect);
        let options = category.map(category => {
    ...
    function btnSearch() {
        ...
        history.pushState({}, null, "/admin/product-type?search=" + btoa(encodeURIComponent(JSON.stringify(object))));
...
<!-- MODAL EDIT-->
<script>
    const editGetBaseUrl = @json(route('admin.productType.editGet', ['id' => 'ID_PLACEHOLDER']));
    const editPostBaseUrl = @json(route('admin.productType.editPost', ['id' => 'ID_PLACEHOLDER']));
    function createDynamicInputs(data) {
        ...
        data.forEach(function(item, index) {
            ...
            if (item.name == "status") {
                ...
                optionData = @json($dataSelect);
                options = optionData.map(cate => `
                    <option value="${cate.id}" ${cate.id == item.value ? 'selected' : ''}>${cate.category_name}</option>
                `).join('');
                var inputHTML = `
                    <div class="form-group mb-2">
                        <label for="input_${index}">${item.label}</label>
...
<script>
    ...
    function onAdd() {
        currentEditId = null;
    $('#inputTitle').text('Thêm Mới Loại Sản Phẩm');
    $('#formModal')[0].reset();
    $('#dynamicInputs').empty();

    const data = [
        { label: 'Tên loại sản phẩm', name: 'product_type_name', type: 'text', value: '', placeholder: 'Nhập tên', setting: '' },
        { label: 'Danh mục', name: 'category_id', type: 'select', value: '', placeholder: '', setting: '' },
    ...
    function saveModalAdd() {
        let formData = $('#formModal').serialize();
        $.ajax({
            url: `{{route('admin.productType.add')}}`,
            type: 'POST',
            data: formData,
            success: function(result) {
            if(result.success){
...
<!-- Delete -->
<script>
    const deleteBaseUrl = @json(route('admin.productType.delete', ['id' => 'ID_PLACEHOLDER']));
