        ...
        <div class="col-12">
            <div class="card w-100">
                <div class="card-body p-4">
                    <a  href="{{route('admin.productType')}}"><h5 class="card-title fw-semibold mb-4">Bảng Danh Mục</h5></a> 
                    ...
                    <button class="btn btn-secondary light "
                        title="Nhấn để tìm kiếm" type="button" onclick="onAdd()" id="saveEdit">Thêm Danh Mục</button>
...
<script>
    ...
    function btnSearch() {
        ...
        history.pushState({}, null, "/admin/category?search=" + btoa(encodeURIComponent(JSON.stringify(object))));
...
<!-- MODAL EDIT-->
<script>
    const editGetBaseUrl = @json(route('admin.category.editGet', ['id' => 'ID_PLACEHOLDER']));
    const editPostBaseUrl = @json(route('admin.category.editPost', ['id' => 'ID_PLACEHOLDER']));
...
<script>
    ...
    function onAdd() {
        currentEditId = null;
    $('#inputTitle').text('Thêm mới danh mục');
    $('#formModal')[0].reset();
    $('#dynamicInputs').empty();

    const data = [
        { label: 'Tên danh mục', name: 'category_name', type: 'text', value: '', placeholder: 'Nhập tên', setting: '' },
...
<!-- Delete -->
<script>
    const deleteBaseUrl = @json(route('admin.category.delete', ['id' => 'ID_PLACEHOLDER']));
